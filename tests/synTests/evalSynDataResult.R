...
for (p in c('argparse', 'dplyr', 'GenomicRanges', 'stringdist', 'stringr')) suppressPackageStartupMessages(library(p, character.only = TRUE))
options(warn=-1)

# Parse command line arguments.
parser <- ArgumentParser()
parser$add_argument("-s", "--sitesFile",    type="character",  default='sites.rds',            help="Path to INSPIIRED2 sites output file (rds).", metavar="")
parser$add_argument("-m", "--multiHitFile", type="character",  default='multiHitClusters.rds', help="Path to INSPIIRED2 multiHitCluster output file (rds).", metavar="")
