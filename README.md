...
- The **anchor read** (`R2`) crosses the vector-genome junction. Vector-terminal sequence anchors the read to the expected vector end, while the remaining sequence identifies the genomic integration position.
- The **adrift read** (`R1`) begins at the ligated linker and reads into genomic DNA from the sonic-shearing boundary. Variation in this boundary provides the primary estimate of clonal abundance.
...
This manual describes the INSPIIRED2 v1.4.4 codebase with the reviewed `demultiplex` and `testHMMs` corrections applied.

Most analyses should begin with the defaults. Change advanced alignment, clustering, HMM, and positional-standardization settings only after reviewing the relevant logs and diagnostic outputs.
...
The supplied Docker image is the recommended execution environment because INSPIIRED2 depends on R/Bioconductor packages and command-line tools including HMMER, BLAT, BLAST+, CD-HIT-EST, samtools, and UCSC sequence utilities.

INSPIIRED2 uses Linux multicore processing. CPU, memory, disk, and shared-memory requirements depend on library size, read complexity, and the reference genomes used. A server with approximately 30 cores and 100 GB RAM is a reasonable starting point for substantial datasets, but these are not hard minimums. On smaller systems, explicitly reduce `--threads`; memory use generally increases with the number of concurrent workers.

Temporary files are written below `--ramDiskPath`, which defaults to `/dev/shm`. If that location is not writable, INSPIIRED2 uses the output directory. The Docker `--shm-size` setting controls the space available in `/dev/shm`.

Check the installed version and command help with:
...
Increase `--shm-size` for larger datasets. To make output files owned by the current host user, `--user "$(id -u):$(id -g)"` may be added. Do not combine this with a `/resources` overlay unless that user can create and replace links below `/opt/INSPIIRED2/data` inside the image.

For reproducible analyses, record the INSPIIRED2 version and immutable container image identifier rather than relying only on the moving `latest` image name.
...
1. Index 1 reads, normally `I1.fastq.gz`.
2. Adrift reads, normally `R1.fastq.gz`.
3. Anchor reads, normally `R2.fastq.gz`.

The files must contain the same read records in the same order. Standard Illumina FASTQ organization is expected. Do not independently sort, subset, or otherwise reorder one mate. Pairing is positional: `demultiplex` cleans the I1 identifier at its first whitespace and assigns that identifier to the corresponding R1 and R2 records; it does not compare the original mate identifiers. Unequal records within a processed chunk are detected, but extra trailing R1/R2 records after I1 reaches end-of-file are not. Verify FASTQ synchronization before running.
...
- Use one `refGenome` for all rows belonging to the same `trial`/`subject` analysis. This also means that a sample must not mix reference genomes; several downstream standardization and grouping operations assume this relationship.
